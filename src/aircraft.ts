...
export abstract class Aircraft {
  constructor(
    ...
    public command: { turn?: { heading: Heading; beacon?: number }; altitude?: number },
  ) {
  }
  ...
  public performCommand(tick: number, map: GameMap) {
    ...
    if (command.turn !== undefined) {
      const { heading, beacon } = command.turn;

      if (beacon !== undefined) {
        const [beaconX, beaconY] = map.info.beacons[beacon];
        const atBeacon = this.x === beaconX && this.y === beaconY;
        if (!atBeacon) return;

        delete command.turn.beacon;
      }

      const cwTurns = (heading - this.heading + HEADING_COUNT) % HEADING_COUNT;
      const ccwTurns = (this.heading - heading + HEADING_COUNT) % HEADING_COUNT;
